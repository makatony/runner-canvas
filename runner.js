class Runner {
  ...
  constructor(x, y) {
    ...
    this.sightDist = 20; //assumes everything beyond 19 pixels is a wall
  }

  update() {
    this.pos.x++;
    this.calcWallDistance();
    if (this.wallDistance[0] == 0) console.log('"dead"');
  }
  ...
  calcWallDistance() {
    ...
    for (let h = 0; h < 4; h++) {
      ...
      for (let d = 0; d < this.sightDist; d++) { //calculating distance to wall based on 4 sights
        ...
        pxlIndex = Math.floor((vector.x + width * vector.y) * pixelDensity() * 4);
